        ...
        <div className="emergency-page page">
            <div className="emergency-wrap">
                <div className="emrg-header">
                    ...
                    <p>Stay calm. Tap to call for assistance.</p>
                </div>

                <div className="quick-calls">
                    {QUICK_CALLS.map((q, i) => (
                        q.num ? (
                            <a href={`tel:${q.num}`} key={i} className="q-call-btn" style={{ '--btn-c': q.c, textDecoration: 'none' }}>
                                <span className="q-call-icon">{q.icon}</span>
                                <div>
                                    <div className="q-call-label">{q.label}</div>
                                    <div className="q-call-num">{q.num}</div>
                                </div>
                            </a>
                        ) : (
                            <button key={i} className="q-call-btn" style={{ '--btn-c': q.c }} onClick={toggleFlash}>
                                <span className="q-call-icon">{q.icon}</span>
                                <div>
                                    <div className="q-call-label">{q.label}</div>
                                    <div className="q-call-num">{flashOn ? 'ON' : 'OFF'}</div>
                                </div>
                            </button>
                        )
                    ))}
                ...
                <div className="emrg-grid">
                    ...
                    <div className="emrg-card card">
                        ...
                        <div className="contacts-grid">
                            {CONTACTS.map((c, i) => (
                                <a href={`tel:${c.phone}`} key={i} className="contact-tile" style={{ '--tile-c': c.c, textDecoration: 'none' }}>
