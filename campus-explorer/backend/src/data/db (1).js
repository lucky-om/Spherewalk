...
function seedDatabase() {
  ...
  const infoCount = db.prepare('SELECT COUNT(*) as cnt FROM campus_info').get().cnt;
  if (infoCount === 0) {
    const insInfo = db.prepare('INSERT INTO campus_info (category, title, content) VALUES (?, ?, ?)');
    const infoRows = db.transaction((rows) => { for (const r of rows) insInfo.run(...r); });
    infoRows([
      ['General', 'Campus Timings', 'College hours: Monday-Saturday 9:00 AM - 6:00 PM. Library open till 8:00 PM.'],
      ['General', 'Wi-Fi Access', 'Campus Wi-Fi SSID: CampusNet. Available across all blocks. Use your student ID to log in.'],
      ['Facilities', 'Canteen Hours', 'Canteen operates from 8:00 AM to 8:00 PM. Special meals available on request.'],
      ['Facilities', 'Sports Complex', 'Ground floor, Sports Block. Open 6:00 AM - 7:00 PM. Includes cricket, football, and badminton courts.'],
      ['Academic', 'Exam Schedule', 'Mid-sem: Week 8. End-sem: Week 16. Results declared within 3 weeks of exams.'],
    ]);
    console.log('✅ Seeded campus_info');
  }
}
