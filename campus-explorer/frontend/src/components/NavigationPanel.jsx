...
import { BUILDING_ROOMS, BUILDING_FLOORS, getRoomsForFloor } from '../pages/navigationGraph';
...
export default function NavigationPanel({ onNavigate, onClose, isOpen }) {
    ...
    useEffect(() => {
        ...
        if (toFloor > maxFloor) setToFloor(0);
    }, [toId]);

    // When floor changes or rooms list changes → pick first room
    useEffect(() => {
        if (rooms.length > 0 && !rooms.includes(toRoom)) {
            setToRoom(rooms[0]);
        }
    }, [toFloor, toId, rooms]);
